	...
	if(sessions && g_hash_table_size(sessions) > 0) {
		...
		while (g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			if ((now - session->last_keepalive) >= SESSION_KEEPALIVE * G_USEC_PER_SEC && !session->timeout) {
				JANUS_LOG (LOG_VERB, "Sending keepalive for session %"SCNu64"...\n", session->session_id);
				if (session->source) {
					json_t * event = json_object ();
...
int janus_process_incoming_response (janus_request *request) {
	...
	if (session_id > 0 && !strcasecmp(message_text,"ack")) {
		...
		if (!session) {
			JANUS_LOG (LOG_ERR, "Couldn't find session %"SCNu64"...\n", session_id);
			ret = janus_process_error (request, session_id, NULL, JANUS_ERROR_SESSION_NOT_FOUND, "No such session %"SCNu64"", session_id);
			goto jsondone;
		}
		session->last_activity = janus_get_monotonic_time();
		ret = 0;
	...
	if (!strcasecmp(message_text,"error")) {
		...
		if (error && json_is_object(error)) {
			...
			if (errorreason && json_is_string (errorreason)) {
				JANUS_LOG (LOG_VERB, "Error message received: %s ..\n",json_string_value(errorreason));
				ret = 0;
				goto jsondone;
			}
		}
...
int janus_process_incoming_request(janus_request *request) {
	...
	if (request->local_message) {
		JANUS_LOG (LOG_VERB, "Handling response from Janus gateway, using local_message\n");
...
void janus_transport_connected (janus_transport * plugin, void * transport) {
	JANUS_LOG (LOG_VERB, "Connected to gateway, registering self...\n");
	char tr[12];
	janus_random_string(12, (char *)&tr);
	json_t * message = json_object();
	json_object_set_new(message, "janus", json_string("create"));
	json_object_set_new(message, "transaction", json_string(tr));
	
	plugin->send_message(transport, NULL, FALSE, message);
}
...
void janus_transport_task(gpointer data, gpointer user_data) {
	...
	if(!request->admin)
		janus_process_incoming_response(request);
	else
...
int janus_plugin_send_request (janus_plugin_session * plugin_session, janus_plugin * plugin, const char * transaction, json_t * message, json_t * jsep, const char * janus_text) {
	...
	if (message) json_incref(message);
