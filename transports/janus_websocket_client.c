...
#define JANUS_WEBSOCKETS_DESCRIPTION		"This transport plugin acts as a client to connect Janus WS server."
#define JANUS_WEBSOCKETS_NAME				"JANUS WebSockets client plugin"
#define JANUS_WEBSOCKETS_AUTHOR				"Meetecho s.r.l."
#define JANUS_WEBSOCKETS_PACKAGE			"janus.transport.wsclient"
...
int janus_websockets_init(janus_transport_callbacks *callback, const char *config_path) {
	...
	if(config != NULL) {
		...
		if(!item || !item->value || !janus_is_true(item->value)) {
			...
			cinfo.host = hostip;
			cinfo.address = hostip;
			cinfo.port = wsport;
			cinfo.path = path;
			cinfo.protocol = "janus-protocol";
#else
		...
		void *user, void *in, size_t len, gboolean admin)
{
	...
	switch(reason) {
		case LWS_CALLBACK_CLIENT_ESTABLISHED: {
			...
			JANUS_LOG(LOG_VERB, "[%s-%p] WebSocket connection established.\n", log_prefix, wsi);
			...
			gateway->transport_connected (&janus_websockets_transport, ws_client);
			return 0;
		}
